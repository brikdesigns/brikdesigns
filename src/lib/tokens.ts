...
export const color = {
  ...
  service: {
    ...
    //
    // `onDark` mirrors `onLight` on the opposite context axis: the component
    // fill (`--background-service-{slug}-on-dark`) to use when the element sits
    // on a known *dark* backdrop. Pairs with the `inverse` surface in dark mode
    // (deep `{hue}-darkest` tint), where a pale on-dark fill reads correctly.
    brand: {
