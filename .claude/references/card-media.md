...
| `customer-topic-grid__media` | `/customers/[slug]` | Not card media. `customer-topic-grid` is a **layout grid** (`display: grid`, `customers.css`), not a card; `__media` is a cover-fit topic illustration in a plain flex `__content` column, not inside any `.bds-card`. There is no card to host a `<Frame>` — the grid also contains a row of BDS `ServiceCard`s, so wrapping it in a `<Card>` would nest cards inside a card. #1191 listed it as a Tier-2 conversion (name matched `[block]__media`); it is an exception. |
| `services-callout-card__media`, `service-sibling-card__media` | `/services*` | The services section deliberately uses a **service-themed `--surface-accent` well**, not the neutral `--surface-secondary` standard. Migrating would erase the theming. **Settled permanent exception** — operator approved 2026-09-05 ("1 + 2 are approved", chat): the themed well stays and these do not normalize to neutral. |
