...
| `./industries.css` | `.industries-hero`, `.industry-card`, `.industry-grid`, `.page-section--industries` / `--brikdown`, `.brikdown-cta*` (index); `.customer-hero-media*`, `.customer-topic-grid*` (detail) | layout only |
...
| 2 | `section.page-section.page-section--industries` — **conditional** on the CMS list | `industries` | `--surface-primary` white | untinted → cards keep border, 3px (`shared-sections.css`) | `Grid columns="auto-fit" minColumnWidth="300px"` (`.industry-grid`, width:100%), `Card layout="stack"` (`.industry-card`), `Frame ratio="square" fit="contain"` |
| 3 | `section.page-section.page-section--brikdown` | `brikdown` | `--surface-primary` white | container's own border (not a band rule) | `.brikdown-cta` 2-col, `LinkButton` → `/offers/brikdown` |
...
**Section 2's grid is `columns="auto-fit" minColumnWidth="300px"`, fed by the CMS row count.** It
reflows to fill the full 1440 column at any count (4 industries → one 4-up row; more wrap gracefully),
so there is no fixed-column gap (the #1004 failure mode) and no shrink-wrap/centering (#1483). The
fill needs `.industry-grid { width:100% }` — BDS `.bds-grid` omits it and `.container-lg` centers,
not stretches, its children.
...
|---|---|---|
| Copy comes from the Notion "Industries" page, not Figma | Figma copy is placeholder/Marketing-flavored ("We already speak your language", "$2,500 Foundation Investment") — [[figma-is-style-not-data]] | `page.tsx` hero + brikdown-cta |
...
| Section 2 is conditional on the CMS list | no published industries → no empty band | `page.tsx` `industryCards.length > 0` |
| Section 2 grid is `columns="auto-fit"` (+ `.industry-grid` width:100%), never a fixed `columns={N}` | fixed columns shrink-wrap/center under `.container-lg` and wrap the 4th card alone — the "narrow section" (operator, 2026-09-12) | `page.tsx:51`, `industries.css` `.industry-grid` |
| BrikDown-CTA image is a **placeholder well** (`.brikdown-cta__media`, no `<img>`) | operator: "placeholder for now" (2026-09-10); no broken-asset request | `page.tsx` §3 |
| Section 3 CTA `href` = `/offers/brikdown`, never `/contact` | site BrikDown convention (home, plans, `about/page.tsx:22`); a button labelled "BrikDown" pointing at `/contact` was a #1406 copy-forward (operator, 2026-09-12) | `page.tsx` §3 `LinkButton` |
| Section 3 block class is `.brikdown-cta`, named for its content role — never the Figma frame name (`section-customer-story`) | a block class must not contradict its own `data-section` (`brikdown`); [[figma-is-style-not-data]] (operator, 2026-09-12) | `page.tsx:86`, `industries.css` `.brikdown-cta` |
