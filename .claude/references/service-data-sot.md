...
- **Single canonical translation function:** [`mapServiceLineSlug()`](../../src/lib/supabase/queries.ts) — short-form line slug → BDS `ServiceLine` enum. Hard-coded sources that bypass it (see [§Hard-coded arrays](#hard-coded-arrays-that-bypass-mapservicelineslug)) are drift risks.
...
| Category color | `mapServiceLineSlug(cat.slug)` → BDS `ServiceTag` category prop |
...
| Per-line color | `mapServiceLineSlug(cat.slug)` |

### `/services/[serviceLineSlug]` (category landing)
...
| 404 path | `getServiceLineBySlug` throws → `notFound()` |

### `/services/[serviceLineSlug]/[serviceSlug]` (service detail)
...
| --- | --- | --- |
| "Other Service Lines" | Service-line landing | `getServiceCategories().filter(c => c.slug !== serviceLineSlug)` — sibling lines, not sibling services |
| "Sibling services" (3-up) | Service detail | `getServicesByServiceLine(serviceLine.id).filter(s => s.slug !== serviceSlug).slice(0, 3)` |
...
| Fallback input | `mapServiceLineSlug(row.slug)` — legacy rows where the column is NULL |
...
## Hard-coded arrays that bypass `mapServiceLineSlug`
...
1. **Never construct a `/services/*` URL from a raw DB slug.** Pass through `mapServiceLineSlug()` for the line slug. See [service-url-slug-convention.md](./service-url-slug-convention.md).
2. **Never add a new hard-coded service-line array** without updating this inventory + the [Hard-coded arrays](#hard-coded-arrays-that-bypass-mapservicelineslug) table.
